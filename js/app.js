...
// Deteksi koordinat yang sebenarnya belum diisi. Titik [0,0] ("null island",\
// tengah lautan dekat Afrika) dipakai sebagai penanda "belum ada koordinat\
// asli" -- di Bali/Indonesia nggak akan pernah ada aset beneran di titik ini.\
...
\}\
\
// Link langsung ke Google Earth Web untuk sebuah koordinat -- ini cuma\
// hyperlink biasa (dibuka browser pengguna), bukan panggilan API dari script.\
...
\}\
\
document.getElementById('filterStatus').addEventListener('change', () => \{ currentPage = 1; renderAll(); \});\
...
function renderPagination(totalItems, totalPages)\{\
  ...
  container.innerHTML = `\
    ...
    <div class="pagination-buttons">\
      <button id="btnPrevPage" $\{currentPage <= 1 ? 'disabled' : ''\}>\'8b Sebelumnya</button>\
      <span class="small-note" style="margin:0;">Halaman $\{currentPage\} / $\{totalPages\}</span>\
      <button id="btnNextPage" $\{currentPage >= totalPages ? 'disabled' : ''\}>Berikutnya \'9b</button>\
    </div>\
  `;\
...
\
  const geomInfo = a.geomType === "point"\
    ? (isValidPoint(a.point) ? `Titik ($\{a.point[0].toFixed(6)\}, $\{a.point[1].toFixed(6)\})` : `Titik \'97 belum ada koordinat`)\
    : (isValidPolygonCoords(a.coords) ? (() => \{\
        const c = computeCentroid(a.coords);\
        return `Poligon ($\{a.coords.length\} titik) \'97 titik tengah: $\{c[0].toFixed(6)\}, $\{c[1].toFixed(6)\}`;\
      \})() : `Poligon \'97 belum ada koordinat`);\
\
  const hasValidCoords = a.geomType === "point" ? isValidPoint(a.point) : isValidPolygonCoords(a.coords);\
  const refCoords = hasValidCoords ? (a.geomType === "point" ? a.point : computeCentroid(a.coords)) : null;\
...
\
  ...
  panel.innerHTML = `\
    ...
    <p class="small-note" style="margin:-4px 0 8px;">\uc0\u8593  Ini yang harus dipakai sebagai <code>asset_id</code> kalau menambah riwayat manual lewat tab Riwayat di Sheets (bukan kode_aset).</p>\
    <div class="view-row"><span class="view-label">Kode aset</span><span class="view-value">$\{escapeHtml(a.props.kode_aset || "-")\}</span></div>\
    <div class="view-row"><span class="view-label">Asal aset</span><span class="view-value">$\{escapeHtml(a.props.asal_aset || "-")\}</span></div>\
    <div class="view-row"><span class="view-label">Lokasi</span><span class="view-value">$\{escapeHtml(a.props.lokasi || "-")\}</span></div>\
    <div class="view-row"><span class="view-label">Luas (m\'b2)</span><span class="view-value">$\{Number(a.props.luas||0).toLocaleString('id-ID')\}</span></div>\
    ...
    $\{earthUrl ? `<div class="view-row"><span class="view-label">Google Earth</span><span class="view-value"><a href="$\{earthUrl\}" target="_blank" rel="noopener" style="color:#1F78B4;">Buka di Google Earth</a></span></div>` : ''\}\
    $\{extraSection\}\
    <div class="field" style="border-top:1px dashed var(--border);padding-top:10px;margin-top:10px;">\
      ...
      <div class="actions-row">\
        <button class="primary" id="btnTambahFoto" style="font-size:12px;">\uc0\u55357 \u56567  Tambah foto</button>\
      </div>\
    </div>\
    <div class="field" style="border-top:1px dashed var(--border);padding-top:10px;margin-top:10px;">\
  ...
  document.getElementById('foto-input').addEventListener('change', async (e) => \{\
    ...
    try\{\
      ...
      \}\
      const res = await addPhoto(\{ asset_id: a.id, base64, mimeType: 'image/jpeg', lat, lng, sumber_tag \});\
...
async function loadAndRenderPhotos(assetId)\{\
  ...
  if(!photos.length)\{\
    ...
    return;\
  \}\
  const sumberLabel = \{ centroid: 'Titik tengah poligon (legal)', gps_hp: 'GPS HP', titik_aset: 'Titik aset', tidak_diketahui: 'Lokasi tidak diketahui' \};\
  stillOpen.innerHTML = `<div class="foto-grid">$\{photos.map(p => `\
    <div class="foto-item">\
      ...
      <div class="foto-meta">\
        ...
        <span class="small-note" style="margin:0;">\uc0\u55357 \u56525  $\{escapeHtml(sumberLabel[p.sumber_tag] || p.sumber_tag || '-')\}</span>\
  ...
  panel.innerHTML = `\
    ...
    <div class="field"><label>Asal aset</label>\
      <select id="f-asal_aset">\
        $\{ASAL_ASET_OPTIONS.map(o => `<option value="$\{o\}" $\{o===a.props.asal_aset?'selected':''\}>$\{o\}</option>`).join('')\}\
      </select>\
    </div>\
    <div class="field"><label>Lokasi</label><input type="text" id="f-lokasi" value="$\{escapeHtml(a.props.lokasi)\}"></div>\
    <div class="row2">\
      <div class="field"><label>Luas (m\'b2)</label><input type="number" id="f-luas" value="$\{a.props.luas\}"></div>\
      ...
      try\{\
        ...
        \} else if(geom.type === "MultiPolygon")\{\
          if(geom.coordinates.length > 1)\{\
            alert("GeoJSON ini berisi " + geom.coordinates.length + " bagian poligon terpisah. Sistem cuma mendukung 1 bagian per aset -- cuma bagian pertama yang dipakai. Kalau bidangnya memang terpisah, tambahkan sisanya sebagai aset baru.");\
          \}\
        ...
        \} else if(geom.type === "LineString" || geom.type === "MultiLineString")\{\
          ...
          if(geom.type === "MultiLineString" && geom.coordinates.length > 1)\{\
            alert("GeoJSON ini berisi " + geom.coordinates.length + " garis terpisah. Cuma garis pertama yang akan dipakai untuk aset ini.");\
          \}\
          line = line.slice();\
          const first = line[0], last = line[line.length - 1];\
          if(first[0] !== last[0] || first[1] !== last[1])\{\
            line.push(first); // tutup batas otomatis (titik akhir disambung ke titik awal)\
          \}\
...
document.addEventListener('click', (e) => \{\
  ...
  if(actions.classList.contains('open') && !actions.contains(e.target) && e.target !== burger)\{\
